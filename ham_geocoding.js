	 function    squareToLatLong(qth) {
	    ...
	    $( "#locator_klick" ).live( "change" , function() {
			...
			if  (qth.length<6) { alert('请输入6个字符的网格代码！') };
		  });
...
function geocode(addr)
{
    ...
    $.ajax({
        ...
        type: 'GET',
        dataType: "json",
        url: "https://nominatim.openstreetmap.org/?addressdetails=1&q="+addr+"&format=json&limit=1",
        data: {},
       success: function(data){
...
return result;		
	...
	function onMapMove(event) {
		...
		$("#dxcluster_map_button").live('click',function(){
				document.location.href = "https://ham.quickso.cn/";
		 });
		 
		 $("#dxcluster_list_button").live('click',function(){
				document.location.href = "https://zh.wikipedia.org/wiki/%E6%A2%85%E7%99%BB%E9%BB%91%E5%BE%B7%E5%AE%9A%E4%BD%8D%E7%B3%BB%E7%BB%9F";
		 });
